...
- `vendor-tools` — upstream's typed vendor schemas + `.passthrough()` at every object level (plus `BillAddr.Line2`/`Line3` typed), so valid QBO Vendor fields outside the schema (`Active`, `PrintOnCheckName`, `AcctNum`, …) aren't silently stripped. Mirrors upstream's own create-journal-entry fix (`bef85c4`) and converges with open upstream PR #108; replaced the former `z.any()` relax 2026-08-07 (DECISIONS § "Vendor schemas: typed + passthrough, not `z.any()`").
