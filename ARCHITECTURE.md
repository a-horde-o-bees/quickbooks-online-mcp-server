...
- `batch_request` — up-to-30-item create/update/delete/query (+ `optionsData` pass-through) over POST `/batch`, input-validated at the tool boundary, healing and re-sending once on a token-expiry 401; the throughput backbone for pushes (chunking and concurrency live in the consumer, `qbo_pipeline._batch`).
- `query_entity` — the **primary read path**: one entity-agnostic paginated `SELECT * FROM <entity> STARTPOSITION n MAXRESULTS k` for any of the 29 entity types the server serves, replacing the per-entity `search_*` tools (whose pagination was inconsistent — some rejected paged criteria; credit-memos/POs couldn't page past 1000). It issues that SELECT over the **POST `/batch`** Query operation, *not* the GET `/query` that node-quickbooks `findX` wraps — so it returns only the live, *referenceable* set, excluding the reset-tombstones `/query` surfaces (read `Active=true` yet fault `2500` when referenced). Every read feeding reference resolution (the pull's `fetch_entity_index` / `iter_record_pages`) goes through it. See DECISIONS § "Reads go through the `/batch` Query endpoint".
- `force-reauth` — `auth-server` clears stale in-memory tokens on re-auth so a fresh consent recovers from invalidated state.
- `vendor-tools` — upstream's typed vendor schemas + `.passthrough()` at every object level (plus `BillAddr.Line2`/`Line3` typed), so valid QBO Vendor fields outside the schema (`Active`, `PrintOnCheckName`, `AcctNum`, `Notes`, …) aren't silently stripped. Mirrors upstream's own create-journal-entry fix (`bef85c4`) and converges with open upstream PR #108; replaced the former `z.any()` relax 2026-08-07 (DECISIONS § "Vendor schemas: typed + passthrough, not `z.any()`").
