...
The push-side `batch_request` handler splits the reactive arm with its caller: on a token-expiry error it **heals the token only** (forced `refreshAccessToken()` + rebuild — QBO is the authority that just rejected it, so the client's own expiry estimate is not trusted) and surfaces the error without re-sending; the **re-send is caller-side** (`qbo_pipeline._batch._dispatch_batch`'s bounded-backoff retry), where mutation retry policy lives and where the idempotency reasoning is ratified — a 401 rejects the whole `/batch` call before the API processes anything, the same pre-execution argument as the 429 carve-out. Without the heal, the caller's re-call would land on the per-call proactive `authenticate()`, which refreshes only when the client's own estimate agrees the token is stale — doomed in exactly the early-expiry mode that motivated the read path's reactive arm. The shared predicate lives in `helpers/token-expiry.ts`.
